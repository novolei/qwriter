...
Agent 生成的 `#knowledge-…` 引用在审阅前转换为普通 Markdown 标题链接，并在文末附上实际引用的来源快照。预览、审阅、采纳、另存及显式继续打磨使用同一份处理内容。用户可以关闭来源保留，此时内部引用变为普通文字；无法匹配本次来源的引用不会伪造来源，而是移除链接并提示核对。历史中的原始结果与来源注册表保持不变。

文末快照含标题、来源、章节、行号、修订和原片段，只保存实际引用的块并去重。片段置于长度安全的纯文本围栏内，不会加载其中的图片或运行 HTML。导出 Markdown 时这些信息随正文保留，分享前可直接审阅或移除。它是引用时的独立副本，不随知识库的更新或归档自动改变，也不表示事实已核实。

富文本编辑器支持鼠标及 Enter 打开标题锚点，兼容中文、格式化标题与重复标题；跳转不增加文稿内容或撤销记录。编辑标题可能使旧链接失效，界面会提示检查目标。外部 Markdown 阅读器的锚点生成规则可能不同，但来源全文仍能独立阅读；复杂语法的源码保护模式保留原始 Markdown。

实现使用现有 unified/remark 解析栈识别语义链接，仅按源位置替换引用，不重新排版整篇正文；标题 ID 使用 [github-slugger](https://github.com/Flet/github-slugger)。原始 Markdown 不存储专有编辑器 ID。验收与固定样本见 [便携引用 audit](audit/2026-09-07-portable-citations/REVIEW.md)。
