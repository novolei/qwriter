...
- `features/agent` 独立管理目标、引用、生命周期、结果和审阅；app 只负责组合。预览使用 react-markdown / remark-gfm，忽略原始 HTML，不请求模型提供的图片或打开外部链接；仅为实际检索到的知识片段注册应用内来源入口。
...
知识库已接入章节/段落分块、FTS5 BM25、CJK 短查询、来源修订与行号定位。Agent 按需检索，不整库预载；来源更新或归档时旧片段 ID 失效，任务历史保留原片段快照。检索到的来源并不自动等同于模型已核实或已引用，详见 [KNOWLEDGE-RETRIEVAL.md](KNOWLEDGE-RETRIEVAL.md)。
