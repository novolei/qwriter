...
4. 查看执行进展和本次阅读记录。Agent 可调用 search_documents / read_document / propose_draft；它无法直接修改文稿。
...
每次最多 6 个模型轮次、每轮最多 8 个工具调用，网络响应最多 2 MB，总对话最多 2 MB。每轮有连接 / 请求超时，可取消连接；服务端是否停止计算取决于具体服务。响应截断、无效工具参数和服务错误不会被当作完整草稿。API Key 不进日志或公开配置，可显式保存在系统凭据库，Agent 网络请求禁止自动重定向。
